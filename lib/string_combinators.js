...
exports.char = string;
exports.anyChar = parser_combinators_2.any;
...
var eosReached = parser_combinators_1.left({ kind: 'pc_error', code: 'eos_reached' });
function string(s) {
    if (!s.length) {
...
function stringInsensitive(s) {
    ...
    return function (st) {
        ...
        return parser_combinators_1.right(parser_combinators_1.pair(acc, cur));
    };
}
...
function oneOfGeneric(s, mapper) {
    if (!s.length) {
        throw new Error('Empty string');
    }
    var charMap = {};
...
function token(p, f, errorMapper) {
    return function (st) {
        var res = p(st);
        if (res.kind === 'left') {
            return parser_combinators_1.left(errorMapper(res.value, st.getPosition()));
        }
        return parser_combinators_1.right(parser_combinators_1.pair(f(res.value[0], st.getPosition(), res.value[1].getPosition()), res.value[1]));
    };
}
exports.token = token;
