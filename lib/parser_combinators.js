...
function separatedTrailing(p, sep) {
    return combine(p, trai(sep, separatedZeroTrailing(p, sep), _2), function (x, xs) { return xs ? (xs.unshift(x), xs) : [x]; });
}
exports.separatedTrailing = separatedTrailing;
function separatedZeroTrailing(p, sep) {
    return combine(many(combine(p, sep, _1)), maybe(p), function (xs, x) { return (x && xs.push(x), xs); });
}
